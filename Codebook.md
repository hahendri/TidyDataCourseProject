...
`subject_test` and `subject_train` are the 30 subjects identified by number

`y_test` and `y_train` are the six activities `as.integer`
...
`X_test` and `X_train` are as follows from `features_info.txt`
